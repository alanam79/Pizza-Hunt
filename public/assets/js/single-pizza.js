...
// retrieve pizza-id data from URL
function getPizza() {
  // get id of pizza
  ...
  fetch(`/api/pizzas/${pizzaId}`)
    .then((response) => {
      ...
      // below will check for a 4xx or 5xx error from server (no pizza found)
      if (!response.ok) {
    ...
    .then(printPizza)
    // below takes user back to homepage, window.history.back acts as a back button essentially
    .catch((err) => {
      console.log(err);
...
// calls the function to retrieve pizza-id data from URL
getPizza();
